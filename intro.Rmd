...
The best and quickest way to understand the simplex tree structure is with a picture. Here's one (taken from the paper) depcisting a simplicial 3-complex (left) and its corresponding Simplex Tree (right). Notice the bijection between the nodes and the face in the complex: there exactly one node for each face, and each node stores exactly one label.

![simplex tree figure](man/figures/simplextree.png)

In what follows, for those interested, I will outline 
...
print(st)
```
Each $k$-simplex is represented as a $(k+1)$-length integer vector containing the labels of the simplex. Inserting simplices into the complex can be done one at a time or specified as a collection via a list, either through the `insert` function or at initialization time via `simplex_tree`. In inserting a simplex $\sigma$, to conform to the properties of being a (abstract) simplicial complex, all faces of $\sigma$ must also being inserted. Thus, its sufficient to specify just the maximal faces, i.e. the simplices which are not themselves faces of any other simplex. For example, the complex above has ten 0-simplices, twelve 1-simplices, six 2-simplices, and one 3-simplex, but to construct it all one needs to do is insert the simplices $\{1,2,3\}$, $\{2,3,4,5\}$, $\{6,7,9\}$, $\{7,8\}$, and $\{10\}$.
...
As a warmup, consider the problem of enumerating all the simplices in the complex. If we have a list of simplices $L = \{ \sigma_1, \sigma_2, \dots, \sigma_m \}$, where each entry contains a an individual. For example, a 2-simplex $\{1, 2, 3\}$  as a full simplicial complex $\mathcal{K}$ might be represented with the list $L = \{1, 2, 3, \{1, 2\}, \{1, 3\}, \{2, 3\}, \{1, 2, 3\} \}$. Since the simplices are already formed, enumerating this list takes linear time $O(m)$, where $m$ is the number of simplices in the list; however, storing this list takes loosely $O(md)$ space, where $d = \mathrm{dim}(\mathcal{K})$ is the dimension of the largest face in the complex. 

Alternatively, we might be in the situation as above where we happen to have the 'largest' simplices on hand whose union captures the full complex. The storage complexity (which is output sensitive) is obviously much lower now, but now enumerating every simplex in the complex require enumerating all lower order faces. For example, using the highly useful `combn` function: 
...
```
This is a nice, compact way of storing a complex, but it has some back assumes that ones has on hand these "largest" simplices. Additionally, there's little structure here: generating the actual vectors containing the labels of each simplex  is requires forming a new simplex at each iteration of the inner loop (via `simplex[i]`). For a $k$-simplex, this requires at least $(k+1)$ operations, leading to a loose complexity of $\approx O(md)$, where again $m = \lvert \mathcal{K} \rvert$ represents is the number of simplices in $\mathcal{K}$ and $d = \mathrm{dim}(\mathcal{K})$ the dimension. 
So one approach uses $O(md)$ storage and then allows $O(m)$ complexity on enumeration, and then other compresses the complex to use $O(m)$ storage, but then requires $O(md)$ time to enumerate. The proverbial "you can't have your cake and eat it too" seems to ring true. 

But now consider the following scenario: enumerating the simplices in the simplex tree in lexicographical order. As mentioned above, this corresponds to a preorder traversal on the trie structure.  
```{r}
## Prints the simplices of each traversal column-wise, i.e. each simplex is a column 
print_simplices(preorder(st), format = "column")
```
Observe that in the sequence of simplices printed by the above _preorder_ traversal, at most one label and one dimension changes per visited node. Imagine keeping just one vector around in memory, which is to be populated with the labels of the current node. In going from a $k$-simplex to a $(k+1)$-simplex, the vector has a new label appended and increases in size by 1, but all other labels are unaffected (and are correct). Similarly, in traversing upwards from $(k+1)$-simplex to a $k$-simplex, the vector shrinks by 1 and has its last label modified. Regardless, the time it takes to form the labels corresponding to a simplex is constant: one size change, and one label change. Thus, enumerating the simplices with a preorder traversal takes linear time $\approx O(m)$ in the size of complex, and since each node stores only one label, we achieve an $\approx O(m)$ size as well.
...
```
The problem: what if $\mathrm{dim}(\mathcal{K}) >> k$? Since the majority of the simplices will have lengths $> k$, the majority of the time will be spent enumerating subtrees corresponding to simplices that will never be needed. This may seem like a trivial concern, but simplicial complexes are frequently built from representations that are inherently combinatorial. One of the unfortunate consequences of this is that they often scale exponentially in size.

Consider the ever popular [Rips complex](https://en.wikipedia.org/wiki/Vietoris%E2%80%93Rips_complex). Say one samples 25 points from the unit square, and then connects these points pairwise with an edge if they are within distance $2 \epsilon$ of each other. The Rips complex is a geometric [flag complex](https://en.wikipedia.org/wiki/Clique_complex), i.e. a simplicial complex whose higher order simplices are determined by the $1$-skeleton entirely. We may recover the Rips up to any dimension by an _expansion_ to the highest order simplicial complex. How quickly does this scale, based on this parameter $\epsilon$?

```{r}
for (eps in seq(0.10, 0.50, by=0.10)){
  set.seed(1234)
  G <- igraph::sample_grg(nodes = 35, radius = eps)
  st <- simplex_tree(as.list(seq(vcount(G)))) %>% insert(as_edgelist(G))
  st %>% expand(k=max(st$degree(st$vertices)))
  cat(sprintf("Parameter=%g:\n", eps))
  print(structure(st$n_simplices, names=seq(0, st$dimension)))
  cat("\n")
}
```
Even though there are only 25 points, by the time the radius parameter is $0.5$, the complex already has over 130 thousand simplices (less than 2\% have dimension $\leq 2$!). 

If one just wants the enumerate the 2-skeleton of the complex, filtering out higher-order simplices isn't enough: the traversal should simply recurse beyond a depth of 2. 
...
For example, consider computing the cofaces of the vertex $\sigma = \{3\}$. Every descendant of the subtree rooted at $\{3\}$ is a coface, so we need only record $3$. Then we check the cousins at a higher depth with the same label. Here's a print out of all the cousins and their corresponding keys (as pairs):
...
st <- simplex_tree(t(edges)) %>% insert(list(7:8, 10))
plot(st, coords=coords, asp = 1)
```
