...
In the realm of Topological Data Analysis (TDA), the beginning of a sometimes large mathematical pipeline is to represent the data as a (abstract) [simplicial complex](https://en.wikipedia.org/wiki/Simplicial_complex). There are many constructions provide either certain theoretical guarentees or computational guarentees, and the choice of construction generally depends on the goal of data analysis: for example, the Čech and Rips are complexes are often used in studying Persistent Homology, nerve complexes (such as the Mapper) and Witness complexes are often used to group areas of the space together to yield computationally simpler representation. Each construction admits a certain intrinsic complexity: for example, the Rips complex at parameter $\epsilon$ admits a face for every subset of points which have diameter less than $\epsilon$, the Witness complex[^2] scales according to the number of specified landmarks, etc. It would be nice to have a simple and elegant data structure for representing each construction efficiently, without being tied to any specific construction. 

Enter the _simplex tree_[^1]: a flexible and efficient data structure for representing and manipulating general simplicial complexes of any dimension. It can effectively be thought of as an ordered [Trie](https://en.wikipedia.org/wiki/Trie), a related tree-like data structure that has proven useful in storing and processing words/text corpora. Like a trie, the simplex tree stores just one 'character' per simplex (+ a constant number of extra pointers to maintain the adjacency structure), associating each node with exactly one face in the complex. Unlike a traditional trie, however, the simplex tree respects a certain ordering requirement wherein children must have labels with values greater than their parents. Additionally, each level beyond dimension $0$ stores additional indexes mapping `cousins', or nodes at the the same label and depth. These relatively simple constraints allow the data structure to scale relatively well in size and particularly in dimension---inline with the intrinsic complexity of construction needed---without restricting the type of construction represented. 

The best and quickest way to understand the simplex tree structure is with a picture. Here's one (taken from the paper) depicting a simplicial 3-complex (left) and its corresponding Simplex Tree (right). Notice the bijection between the nodes and the face in the complex: there exactly one node for each face, and each node stores exactly one label.

![simplex tree figure\label{st_pic}](man/figures/simplextree.png)

In what follows, for those interested, I will discuss some of the basic algorithms on the simplex tree, their associated complexities, and how to use them with the `simplextree` package. Also, I will outline various aspects of the simplex tree which I've found useful in developing my own algorithms.

## Getting started 

To get started with the `simplextree` package, one needs a way of constructing a simplicial complex. One option is to start with an empty complex and build it manually.
...
print(st)
```
in _R_, Each $k$-simplex is represented as a $(k+1)$-length integer vector containing the labels of the simplex. Inserting simplices into the complex can be done one at a time or specified as a collection via a list, either through the `insert` function or at initialization time via `simplex_tree`. In inserting a simplex $\sigma$, to conform to the properties of being a simplicial complex, all faces of $\sigma$ must also being inserted. Thus, its sufficient to specify just the maximal faces, i.e. the simplices which are not themselves faces of any other simplex. For example, the complex above has ten 0-simplices, twelve 1-simplices, six 2-simplices, and one 3-simplex, but to construct it all one needs to do is insert the simplices $\{1,2,3\}$, $\{2,3,4,5\}$, $\{6,7,9\}$, $\{7,8\}$, and $\{10\}$.
...
print(st2)
```
Notice that modifying `st` after also affects `st2`. This happens because `<-` does not create a copy in this case; `st` and `st2` refer to the same object. You can create a new copy with e.g. `st2 <- simplex_tree(st$serialize())`. Note this only copies the simplices themselves; nothing else. This allows many operations which modify the complex to be chained, e.g. another way would've been to do something like `st %>% insert(c(1,2,3)) %>% remove(c(2,3)) %>% ...`, etc.
...
In this definition, $\sigma$ is itself both face and coface of itself. As shown above, enumerating the faces of a simplex is easy enough: given a $(d+1)$-simplex, one can enumerate all $k$-length combinations where $k \leq d + 1$ with `combn`: 
```{r}
sigma <- c(1,2,3,4)
d <- length(sigma)
...
```
Sure enough, this works! In fact, we can even shortcut this to make it more efficient: for each $\sigma \in \mathcal{K}$, rather than enumerating all the cofaces, it is enough to just check whether $\sigma$ has _any_ cofaces aside from itself. If $\sigma$ has any children, it is not maximal, otherwise one just need to check if the label of $\sigma$ exists at a depth greater than $\mathrm{depth}(\sigma)$. If $\mathrm{dim}(\mathcal{K}) = d$ and $\mathrm{dim}(\sigma) = k$, this takes $O((d - k)\log(m))$ checks. Usually $\lvert \mathcal{K} \rvert >> \mathrm{dim}(\mathcal{K})$, effectively this a constant time operation. As as shown above, the preorder enumeration takes time linear in the size of the complex $m = \lvert \mathcal{K} \rvert$. As a result, finding the maximal faces of a given complex reduces to a linear time operation. You can see the maximal simplices using the `maximal` traversal:
...
Consider detecting the clique ${1, 2, 3}$. Starting at vertex $\sigma = \{1\}$, we start by detecting $\sigma$'s neighbors. By the construction of the simplex tree, if another vertex $\sigma'$ is adjacent to $\sigma$ and its label is greater than $\sigma$'s label, it must be a child of $\sigma$. We know that we can form the 2-simplex ${1, 2, 3}$ if and only if we have all edges accounted for; we see that edges $\{1,2\}$ and $\{1,3\}$ are accounted for, and since no simplex has a label greater than $1$ this covers all of $\{1\}$'s edges. We descend to recurse at the node representing the simplex $\{1, 2\}$. The label is $2$. The neighbors of the vertex $\{2\}$ whose labels are greater than $2$ are exactly the children, again: so we intersect the previously accounted labels ($2$ and $3$) with $\{2\}$'s children labels. If there is a non-empty intersection, we know we have detected a clique. Since the labels of any set of children are ordered, this problem computationally reduces to computing the intersection between two sorted ranges, which can be efficiently done (in time linear in the size of the intersection) using e.g. (std::set_intersection)[https://en.cppreference.com/w/cpp/algorithm/set_intersection]. 

For every detected $k$-clique, if the complex is a flag complex, we may insert a $(k-1)$-simplex back into complex, thus 'expanding' the complex. Doing this for all simplices in the complex is what's called _expansion of order k_, or _k-expansion_ for short.

```{r}
maximal1 <- as.list(st %>% maximal())
maximal2 <- as.list(st %>% expand(k = 2) %>% maximal())

print_simplices(setdiff(maximal1, maximal2))
```
...
The Rips complex is a very common 
