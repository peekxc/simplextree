...
Often in the realm of Topological Data Analysis (TDA), the foundational starting point in moving from a "data set" to some type of analysis is the construction of an (abstract) [simplicial complex](https://en.wikipedia.org/wiki/Simplicial_complex). Many such methods of constructions offer certain theoretical or computational guarantees, with the choice of representation depending on the goal of the analysis. For example, the Čech and Rips are complexes are common choices when studying Persistent Homology due to their theoretical guarentees related to homotopy equivalence. Nerve complexes (e.g. the Mapper construction) and Witness complexes[^2] are preferred when areas of the space can be grouped up meaningfully to yield a simpler, more computationally feasible representation. Delaunay complexes are generally used more geometric settings, such as in mesh-generation, etc. 

Each construction admits a certain intrinsic complexity: the Rips complex at parameter $\epsilon$ admits a face for every subset of points which have diameter less than $\epsilon$, the Witness complex scales according to the number of specified landmarks, etc. 


Although there are many difference in what is preserved amongst these constructions, whats common among them all is that their combinatorial descriptions can all be represented as abstract simplicial complexes. It would be nice to have a simple, easy-to-modify data structure capable of representing any simplicial complex, regardless of the chosen construction process. 
...
The simplex tree can be effectively be thought of as an ordered [Trie](https://en.wikipedia.org/wiki/Trie), a related tree-like data structure that has proven useful in storing and processing words/text corpora. Like a trie, the simplex tree stores just one 'character' per node (+ a constant number of extra pointers to maintain the adjacency structure), and each node is associated with with exactly one face in the complex. Unlike a traditional trie, the simplex tree demands a certain ordering invariant be kept wherein parents only keep children whose labels have values greater than their own. Additionally, each level beyond dimension $0$ stores indexes mapping `cousins', or nodes at the the same label and depth (shown by the dotted lines connected $5$-nodes together). These relatively simple modifications give the trie a richer structure, one that ameneable to modification and scales inline with the intrinsic complexity of corresponding complex, regardles of the construction method. 
...
A *traversal* is an ordered path through a subset of the simplices in the simplex tree. If simplices are the atoms from which the simplex tree is created, then traversals are the building blocks from which algorithms can be designed. Traversals form the foundation for many other auxiliary methods---they often provide an elegant starting point when one is tasked with extracting or manipulating parts of the complex. 
...
Straightforward. On the simplex tree, a preorder traversal can be printed concisely with: 
...
Observe that due to the naturally ordered structure of the simplex tree, the preorder traversal corresponds to a [lexicographical ordering](https://en.wikipedia.org/wiki/Lexicographical_order) of the simplices in the complex. Similarly, the level order (not shown) corresponds with the [shortlex](https://en.wikipedia.org/wiki/Shortlex_order) order (i.e. lexicographical per dimension). A related property one can extract from this observation is that every ancestor simplex (a simplex with children) in the trie must be a face of any of its descendants, and similarly every descendant simplex must be a coface of its ancestors. This naturally constructive property can come in handy in crafting efficient algorithms to manipulate the complex. Let's see some examples of this in action. 


### Fundamental building blocks

Consider the problem of enumerating all the simplices in the complex. The complexity of this operation depends on how each simplex is represented in memory. For example, the simplest approach might be to represent every simplex explicitly, collecting all simplices as a simple list of simplices $L = \{ \sigma_1, \sigma_2, \dots, \sigma_m \}$. For example, the full combinatorial description of the 2-simplex $\{1, 2, 3\}$ might be represented with the list $L = \{ \{1\}, \{2\}, \{3\}, \{1, 2\}, \{1, 3\}, \{2, 3\}, \{1, 2, 3\} \}$. Since the simplices are already formed, accessing any element of the list is $O(1)$, and thus enumerating this list takes linear time $O(m)$, where $m$ is the number of simplices in the list; however, the labels representing the simplices are increasingly redundant---each $k$-simplex requires $k+1$ storage, and each simplex has $2^{d+1} - 1$ faces; thus, the number of faces scales exponentially with dimension. An easy upper bound for the size of this list is $\approx O(md)$, where $d = \mathrm{dim}(\mathcal{K})$ is the dimension of the largest face in the complex.
