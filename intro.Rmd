...
Often in the realm of Topological Data Analysis (TDA), the foundational starting point in moving from a "data set" to some type of analysis is the construction of an (abstract) [simplicial complex](https://en.wikipedia.org/wiki/Simplicial_complex). A _simplicial complex_ $\mathcal{K}$ defined over a finite vertex set $V$ is a set of non-empty subsets of $V$ satisfying the following two conditions:

1) $$ p \in V \implies \{p\} \in \mathcal{K} $$
2) $$ \sigma \in \mathcal{K}, \tau \subseteq \sigma \implies \tau \in \mathcal{K}$$

Another way of stating the second condition is that every simplex in the complex is _closed under taking faces_. Each element $\sigma in \mathcal{K}$ is called a simplex or a _face_ of $\mathcal{K}$. If $\sigma \in \mathcal{K}$ and $\sigma$ is a $k$-simplex, then $\sigma$ is made up of exactly $k+1$ vertices (or 'labels'), and the dimension of $\sigma$ is $k$. The dimension of the simplicial complex $\mathcal{K}$ is the largest integer $d$ such that $\mathcal{K}$ contains a $d$-simplex. 

There are many ways of constructing of a simplicial complex, each of which offer certain theoretical or computational guarantees. The choice of representation depends on the goal of the analysis. For example, the Čech and Rips constructions are common choices when studying Persistent Homology due to their theoretical guarantees related to homotopy equivalence. Nerve complexes (e.g. the Mapper construction) and Witness complexes[^2] are often used when a more computationally feasible representation is preferred. Delaunay complexes are often used in more geometric settings, such as in mesh-generation, 3D graphics, etc. Each construction admits a certain intrinsic complexity: the Rips complex at parameter $\epsilon$ admits a face for every subset of points which have diameter less than $\epsilon$, the Witness complex scales according to the number of specified landmarks, etc. Whats common among them all these constructions is that their combinatorial aspects can all be represented as abstract simplicial complexes: they all obey the two properties given above. It would be nice to have a simple and elegant data structure capable of representing any simplicial complex, regardless of the chosen construction process. 

Enter the _simplex tree_[^1]: a flexible data structure for representing and manipulating general simplicial complexes of any dimension. The best and quickest way to understand the simplex tree structure is with a picture. Here's one (taken from the paper) depicting a simplicial 3-complex (left) and its corresponding Simplex Tree (right). Each node is in bijection with the faces in the complex: there exactly one node for each face, and each node stores exactly one label.

![simplex tree figure\label{st_pic}](man/figures/simplextree.png)

The simplex tree can be effectively be thought of as an ordered [Trie](https://en.wikipedia.org/wiki/Trie), a related tree-like data structure that has proven useful in storing and processing words/text corpora. Like a trie, the simplex tree stores just one 'character' per node (+ a constant number of extra pointers to maintain the adjacency structure), and each node is associated with with exactly one face in the complex. Unlike a traditional trie, the simplex tree requires a few additional invariants be kept to be called a simplex tree: 

1. Each level in the trie is _ordered_
2. Child node labels are strictly greater than their parents 
3. Each level beyond dimension $0$ stores additional adjacency relations connecting `cousins', or nodes with the same label and depth. 

An example depicting the 3rd property is illustrated above by the dotted lines connected together the $5$-nodes. These relatively simple modifications give the trie a richer structure, one that is amenable to modification and that scales inline with the intrinsic complexity of corresponding complex.  
...
In inserting a simplex $\sigma$, to conform to the properties of being a simplicial complex, all faces of $\sigma$ are also inserted. As a result, its sufficient to specify just the _maximal faces_, i.e. the simplices which are not themselves faces of any other simplex. The same complex above can be constructed more concisely with: 
```{r}
st <- simplex_tree()
insert(st, c(1,2,3))
print_simplices(st)
...
As another example, consider the complex shown in the figure depicting the data structure. The complex above has ten 0-simplices, twelve 1-simplices, six 2-simplices, and one 3-simplex; these simplices need not be added one by one. All one needs to do is insert the simplices $\{1,2,3\}$, $\{2,3,4,5\}$, $\{6,7,9\}$, $\{7,8\}$, and $\{10\}$.
...
The `print_simplices` function gives you multiple ways of printing the simplices in the complex. You can use the _tree_ and _cousins_ options to see the structure of the underlying simplex tree, the former showing the subtrees rooted at each vertex per line (top-down), and the latter showing how the simplices are stored per level (from the side), for example:
```{r}
print_simplices(st, "tree")
```
To visualize the complex, simply call the `plot` method. The default method uses a force-directed layout algorithm to compute the coordinates in the plot, but you also provide your own:
...
A *traversal* is an ordered path through a subset of simplices in the simplex tree. If simplices are the atoms from which the simplex tree is created, then traversals are the building blocks from which efficient algorithms can be designed. Traversals form the foundation for many other auxiliary methods---they often provide an elegant starting point when one is tasked with extracting or manipulating parts of the complex, such as faces or cofaces. 

As an initial example, consider the classical _preorder_ (or "depth-first") traversal:
...
Straightforward. On the simplex tree, the simplices along a preorder traversal can be printed concisely with: 
...
Observe that due to the naturally ordered structure of the simplex tree, the preorder traversal corresponds with a [lexicographical ordering](https://en.wikipedia.org/wiki/Lexicographical_order) of the simplices in the complex. Similarly, the level order traversal (not shown) corresponds with a [shortlex](https://en.wikipedia.org/wiki/Shortlex_order) ordering (i.e. lexicographical per dimension) of the simplices. A related property one can extract from this observation is that every _ancestor_ simplex (i.e. a simplex with children) in the trie must be a _face_ of any of its descendants. Similarly, every _descendant_ simplex must be a _coface_ of its ancestors. This naturally constructive property can come in handy in crafting efficient algorithms to manipulate the complex, and to do many basic operations on complex as a whole. Let's see some examples of this in action. 


### Fundamental building blocks

For a moment, forget about the simplex tree, and consider the problem of enumerating all the simplices in an abstract simplicial complex. You can choose how to represent the complex in memory--the time and space complexities of any operation on the complex inherently depend on this choice. The simplest approach might be to represent every simplex explicitly, collecting all simplices as into a list $L = \{ \sigma_1, \sigma_2, \dots, \sigma_m \}$. For example, the full combinatorial description of the 2-simplex $\{1, 2, 3\}$ is represented by the list $L = \{ \{1\}, \{2\}, \{3\}, \{1, 2\}, \{1, 3\}, \{2, 3\}, \{1, 2, 3\} \}$. Since the simplices are already formed, accessing any element of the list is $O(1)$, and thus enumerating the simplices in the complex takes linear time $O(m)$, where $m$ is the number of simplices in the list. However, consider the space tradeoff: although we have constant-time access to any simplex, the simplex labels are increasingly redundant, leading to suboptimal storage complexity. Since each $k$-simplex requires $k+1$ storage, and each simplex has $2^{d+1} - 1$ faces, one can conclude that the storage complexity scales linearly in the number of faces, and exponentially in the dimension of the complex. An easy upper bound for the size of this list is $\approx O(md)$, where $d = \mathrm{dim}(\mathcal{K})$ is the dimension of the largest face in the complex.

At the other extreme, suppose we have happen to have the *maximal* simplices on hand (as shown above), i.e. the largest simplices whose union captures the full complex. For example, going back to the example from the picture\ref{st_pic}, one can store just the maximal faces and then enumerate the lower-order ones with the highly useful `combn` function:
...
```
At first glace, this is a nice and compact solution to representing a complex---but it has some drawbacks. Firstly, it assumes that one already has the maximal simplices on hand. Moreover, non-maximal faces must be generated on the fly (shown above using `simplex[i]`): generating a $k$-face from a higher order face in this way requires at least $O(k + 1)$ operations, as opposed to the $O(1)$ access time with the simple list representation. Since each face requires at most $d = \mathrm{dim}(\mathcal{K})$ operations to access, a worst-case complexity estimate for enumerating all the simplices in the complex is $\approx O(md)$.

The simple list approach uses $O(md)$ storage, but allows $O(1)$ access to a simplex, and thus $O(m)$ complexity to enumerate all simplices. The other approach compresses the complex to use a much smaller amount of storage (roughly $\approx O(kd)$, where $k$ is the number of maximal simplices), but then requires at most $O(d)$ access time for any non-maximal simplex. If the number of vertices $n$ is fixed, then as the dimension of the complex $d$ grows, the number of maximal simplices tends to decrease, until we have a full simplicial $d$-complex. In this case, we have $O(d)$ storage, but now every simplex requires at most $O(d)$ time to access if the approach using `combn` is used above. If there are $m$ simplices, the enumeration problem reduces to $O(md)$ complexity. At first glance, this tradeoff between time and space complexity seems unavoidable.
...
print_simplices(preorder(st), format = "column")
```
Observe that in the sequence of simplices printed above, at most one label and at most one dimension changes per visited node. Imagine that during the traversal, one keeps a single vector of length at most $d$ around in memory, whose last entry is to be iteratively modified with the label of the current node in the traversal. In going from a $k$-simplex to a $(k+1)$-simplex, the vector has a new label appended and increases in size by 1---all other labels are unaffected (and are correct). Similarly, in traversing upwards in the trie (descending in dimension) from $(k+1)$-simplex to a $k$-simplex, the vector shrinks in size by 1 and again has its last label modified. Regardless, the time it takes to form the labels corresponding to any individual simplex is _constant_: one size change, and one label change. The complexity of enumerating the simplices is the sum of the complexities of accessing each simplex: thus, enumerating the simplices with a preorder traversal takes _linear time_, $\approx O(m)$, in the size of complex. And, since each node stores only one label, we achieve an $\approx O(m)$ size as well^[This is admittedly a bit of an oversimplification. It is true that the storage complexity of the simplex tree is linear in the size of the complex: if there are $m$ simplices in the complex, then there are $m$ nodes in the simplex tree. However, here I'm using the word RAM interpretation in reporting the complexities; a more accurate description is that the simplex tree requires $\Theta(m \log n)$ space/bits, since each label requires $\Theta(\log n)$ bits to be represented.].

In summary, ordered _traversals_ on the simplex tree may confer certain computational advantages that simplify common operations on a simplicial complex. The example above is perhaps the simplest such operation: enumerating all simplices in the complex. In the next couple sections, I'll demonstrate how other types of traversals provide computationally attractive "building blocks" for performing other types of common operations on the complex.
...
Each _traversals_ in the `simplextree` package is specified independently at the package-level. The first argument to every traversal is the complex to traverse through; other arguments, where applicable, parameterize certain aspects of the traversal. Given a parameterized traversal, one can use the `traverse` function to apply a function on each simplex. The general pattern is 

```
st <- simplex_tree(...)
traverse(traversal_type(st, ...), function(simplex){ ... })
```

As a quick example, consider being tasked with a slightly modified version of the enumeration problem above: create a function to collect the set of simplices of dimension $\leq k$ (i.e. the $k$-skeleton of $\mathcal{K}$) into a list. How would you do it?  
...
```
The problem: what if $\mathrm{dim}(\mathcal{K}) >> k$? Since the majority of the simplices will have lengths $> k$, the majority of the time will be spent enumerating subtrees corresponding to simplices that will never be needed. This may seem like a trivial concern from a user-perspective, but simplicial complexes are frequently built from representations that are inherently combinatorial. One of the unfortunate consequences of this is that they often scale exponentially in the size of input, and as a result what appears to be a trivial concern may turn out to be a very nontrivial computational problem.

Consider the popular [Rips complex](https://en.wikipedia.org/wiki/Vietoris%E2%80%93Rips_complex), which is a geometric [flag complex](https://en.wikipedia.org/wiki/Clique_complex), i.e. a simplicial complex whose higher order simplices are determined by the $1$-skeleton entirely. An easy way of getting a random Rips complex is to first form a graph by e.g. sampling $n$ points from the unit square, connecting points pairwise with an edge if they are within distance $2 \epsilon$ of each other. The Rips complex up to dimension $k$ can be recovered by inserting higher order cliques via e.g. an _expansion_ algorithm. A natural question to ask is how quickly does the complex grow based on the parameter $\epsilon$?

```{r}
set.seed(1234)
for (eps in seq(0.10, 0.50, by=0.10)){
  G <- igraph::sample_grg(nodes = 35, radius = eps)
  st <- simplex_tree(as.list(seq(igraph::vcount(G)))) %>% insert(t(igraph::as_edgelist(G)))
...
```{r}
st <- pbgrad::r_geometric_complex(15, 0.20, dim = 2, coords = TRUE)
xy <- attr(st, "coords")


## Use mod 2 + existing fields to calculate characteristic 
euler_characteristic <- function(st){
  sum(st$n_simplices*((-1)^(0:st$dimension %% 2)))
}
print(euler_characteristic(st))
```

```{r}
## Helper function to calculate lower-star function values 
vertex_heights <- structure(xy[,2], names = st$vertices)

## Create the lower-star structure weights, in shortlex order
fy <- straverse(level_order(st), function(simplex){ 
  max(vertex_heights[as.character(simplex)]) 
})

## Plot the lower-stars w/ function values less than the median
thresholds <- quantile(xy[,2], probs = c(0.20, 0.40, 0.60, 0.80))
layout(matrix(1:4, ncol = 2, byrow = TRUE))
for (h in thresholds){
  plot(st, coords = attr(st, "coords"), color_pal = ifelse(fy <= h, "red", "gray"))
  abline(h =  h, col = "red", lty = 2, lwd = 2)
}
```

```{r}
## Choose a set of thresholds
thresholds <- quantile(fy, probs = seq(0, 1, length.out = 20))

## How many 
tabulate(straverse(k_simplices(st, k = 0), function(simplex){ 
  f <- max(vertex_heights[as.character(simplex)]) 
  findInterval(f, thresholds)
}))

sapply(seq(st$dimension+1), function(d){ 
  cumsum(tabulate(findInterval(fy_ordered[fd_ordered == d], thresholds)))
})

# ## Helper functions to calculate lower-star function values 
# vertex_height <- structure(xy[,2], names = st$vertices)
# lower_star <- function(simplex){ max(vertex_height[as.character(simplex)]) }
# simplex2str <- function(simplex){ paste0(as.character(simplex), collapse = ",") }
# 
# ## Create the lower-star structure weights, in shortlex order
# shortlex <- level_order(st)
# fy <- structure(
#   .Data = straverse(shortlex, lower_star), 
#   names = straverse(shortlex, simplex2str)
# )
# 
# ## Plot the lower-stars w/ function values less than the median
# thresholds <- quantile(xy[,2], probs = c(0.20, 0.40, 0.60, 0.80))
...
```{r}
fy_max <- fy
rev_shortlex <- rev(as.list(shortlex))
for (simplex in rev_shortlex){
  k <- length(simplex)
  if (k > 1){
    proper_faces <- combn(simplex, k-1, simplex2str)
    fy_max[proper_faces] <- fy_max[simplex2str(simplex)]
  }
}
...
layout(matrix(1:4, ncol = 2, byrow = TRUE))
for (h in thresholds){
  plot(st, coords = attr(st, "coords"), color_pal = ifelse(fy_max <= h, "red", "gray"))
  abline(h =  h, col = "red", lty = 2, lwd = 2)
}
```

Suppose we want to compute the Euler characteristic on a sliding window 

```{r}
str2simplex <- function(simplex){ paste0(as.character(simplex), collapse = ",") }

simplices <- lapply(names(fy[fy <= 0.40]), function(s){ as.integer(unlist(strsplit(s, ","))) })
...
## Get the simplex function values and dimension, ordered by weight
fy_ordered <- fy[order(fy)]
fd_ordered <- rep.int(seq(st$dimension+1), times = st$n_simplices)[order(fy)]

## Bin simplices into buckets by dimension, then cumulatively count the size of the buckets 
counts_per_dimension <- sapply(seq(st$dimension+1), function(d){ 
  cumsum(tabulate(findInterval(fy_ordered[fd_ordered == d], thresholds)))
})

## Euler Characteristic Curve
modulus <- ((-1)^(0:st$dimension %% 2))
ecc <- colSums(t(counts_per_dimension)*modulus)

plot(ecc, type = "l", main = "")
```


```{r}
# simplex_to_string <- function(simplex){ paste0(as.character(simplex), collapse = ",") }
# keys <- 
# 
# fx <- structure(rep(0, length(keys)), names = keys)
# 
# fx[as.character(st$vertices)] <- xy[,2]

# euler characteristic on a sliding window
...
Note that `insert` inserts the given simplex(es), and then returns the complex invisibly. That is, `insert`, `remove`, `collapse`, and many other functions enact *side-effects*, wherein the underlying object (the simplicial complex) is changed. No copies are made. This can be surprising to some `R` users, who are used to each and every line effectively being independent from the last, or who are used to being able to isolate changes with the assignment operator. For example, consider:
...
print(st2)
```
Notice that modifying `st` after also affects `st2` (and vice versa!). This happens because `<-` does not create a copy in this case; `st` and `st2` are [references](https://en.wikipedia.org/wiki/Reference_(computer_science)) to the same underlying object. You can create a new copy with e.g. `st2 <- clone(st)`. Note this only copies the simplices themselves; nothing else. 

Methods which modify the underlying `simplextree` object follow the same pattern: the first argument is always the complex, and the function always returns the modified object [invisibly][https://www.rdocumentation.org/packages/base/versions/3.6.2/topics/invisible]. This allows many operations which modify the complex to be chained, e.g. yet another way of achieving the insertions above would be to do something like `st %>% insert(c(1,2,3)) %>% insert(c(2,3,4,5)) %>% ...`, etc. This pattern is known as [method chaining](https://adv-r.hadley.nz/r6.html#method-chaining), and has grown commonplace in the [tidyverse](https://www.tidyverse.org/) collection of packages. 