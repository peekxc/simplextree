...
```{r}
st <- r_geometric_complex(15, 0.20, dim = 2, coords = TRUE)
xy <- attr(st, "coords")
...
traverse(shortlex, function(simplex){
  k <- length(simplex)
  combn(simplex, k-1, simplex2str) lower_star
  if (k > 1){
    proper_faces <- combn(simplex, k-1, simplex2str)
    fy[simplex2str(simplex)] <<- max(fy[proper_faces])
  }
})
...
```{r}
traverse(maximal(st), function(simplex){
  k <- length(simplex)
  if (k > 1){
    proper_faces <- combn(simplex, k-1, simplex2str)
    fy[simplex2str(simplex)] <<- max(fy[proper_faces])
...
forder <- order(fy)
fdim <- rep.int(seq(st$dimension+1), times = st$n_simplices)
fdim <- fdim[forder]
fy <- fy[forder]

counts_per_dimension <- sapply(seq(st$dimension+1), function(d){ 
  cumsum(tabulate(findInterval(fy[fdim == d], thresholds)))
})


## Compute the Euler Characteristic Curve
modulus <- ((-1)^(0:st$dimension %% 2))
ecc <- colSums(t(counts_per_dimension)*modulus)

plot.ecdf(ecc, verticals = TRUE)
```
