...
Another way of stating the second condition is that every simplex in the complex is _closed under taking faces_. Each element $\sigma in \mathcal{K}$ is called a simplex or a face of $\mathcal{K}$. If $\sigma \in \mathcal{K}$ and $\sigma$ is a $k$-simplex, then $\sigma$ is made up of exactly $k+1$ vertices (or 'labels'), and the dimension of $\sigma$ is $k$. The dimension of the simplicial complex $\mathcal{K}$ is the largest $d$ such that $\mathcal{K}$ contains a $d$-simplex. 

There exist many construction methods which offer certain theoretical or computational guarantees, with the choice of representation depending on the goal of the analysis. For example, the Čech and Rips are complexes are common choices when studying Persistent Homology due to their theoretical guarantees related to homotopy equivalence. Nerve complexes (e.g. the Mapper construction) and Witness complexes[^2] are preferred when a more computationally feasible representation is preferred. Delaunay complexes are often used in more geometric settings, such as in mesh-generation, 3D graphics, etc. Each construction admits a certain intrinsic complexity: the Rips complex at parameter $\epsilon$ admits a face for every subset of points which have diameter less than $\epsilon$, the Witness complex scales according to the number of specified landmarks, etc. Whats common among them all these constructions is that their combinatorial aspects can all be represented as abstract simplicial complexes: they all obey the first two properties given above. It would be nice to have a simple and elegant data structure capable of representing any simplicial complex, regardless of the chosen construction process. 
...
2. Parents only keep children whose labels have values greater than their own
...
An example depicting the 3rd property is illustrated above by the dotted lines connected together the $5$-nodes. These relatively simple modifications give the trie a richer structure, one that is amenable to modification and that scales inline with the intrinsic complexity of corresponding complex, regardless of the construction method. 
...
In inserting a simplex $\sigma$, to conform to the properties of being a simplicial complex, all faces of $\sigma$ are also inserted. Thus, its sufficient to specify just the maximal faces, i.e. the simplices which are not themselves faces of any other simplex. Thus the same mplex above can be constructed with: 
```{r}
st <- simplex_tree()
insert(st, c(1,2,3))
print_simplices(st)
...
As another example, consider the complex shown in the figure depicting the data structure. The complex above has ten 0-simplices, twelve 1-simplices, six 2-simplices, and one 3-simplex, but to construct it all one needs to do is insert the simplices $\{1,2,3\}$, $\{2,3,4,5\}$, $\{6,7,9\}$, $\{7,8\}$, and $\{10\}$.
...
```
At first glace, this is a nice and compact solution to representing a complex---but it has some drawbacks. Firstly, it assumes that one already has the maximal simplices on hand. Moreover, non-maximal faces must be generated on the fly (shown above using `simplex[i]`): generating a $k$-face from a higher order face in this way requires at least $(k+1)$ operations. Simplifying a bit, since each face requires at most $d = \mathrm{dim}(\mathcal{K})$ operations to access, enumerating all the simplices in the complex now takes $\approx O(md)$.


So one approach uses $O(md)$ storage and then allows $O(m)$ complexity on enumeration, and the other compresses the complex to use $O(m)$ storage, but then requires $O(md)$ time to access all the faces. The proverbial "you can't have your cake and eat it too" seems to ring true. 
...
Each traversal requires as its first argument the complex through which to traverse. Some traversals support other arguments, e.g. where to start the traversal from, which _parameterizes_ the traversal. Given a parameterized traversal, one can use the `traverse` function to apply a function on each simplex. I'll cover the interface of various traversals later. 
